...
function s.cfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x296) and c:IsType(TYPE_LINK) and c:IsControler(tp)
end
...
function s.spfilter2(c,fg,minmat,maxmat)
	return  c:IsSetCard(0x296) and c:IsLinkBelow(3) and c:IsLinkSummonable(nil,fg,minmat,maxmat)
end
function s.rescon(sg,e,tp,mg)
	return Duel.GetMatchingGroupCount(s.spfilter2,tp,LOCATION_EXTRA,0,nil,sg,#sg,#sg)>0
end
function s.filtercheck(c,e,tp)
	return c:IsCanBeLinkMaterial() and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsSetCard(0x296)
