...
function s.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:GetHandler():GetFlagEffect(id)==0 and
		Duel.IsExistingTarget(aux.FilterFaceupFunction(aux.nzatk),tp,0,LOCATION_MZONE,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectTarget(tp,aux.FilterFaceupFunction(aux.nzatk),tp,0,LOCATION_MZONE,1,1,e:GetHandler())
