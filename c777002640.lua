...
function s.initial_effect(c)
	...
	--Fusion Summon
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,777002500,777002510)
...
function s.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsFaceup() and c:GetDefense()>=750 end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		...
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,777002500) 
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,777002510) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_DECK,0,nil,e,tp,777002500)
	local g2=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_DECK,0,nil,e,tp,777002510)
