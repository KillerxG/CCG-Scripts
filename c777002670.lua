...
function s.spcfilter(c,ft)
	return c:IsAbleToGraveAsCost() and c:IsSetCard(0x291) and c:IsType(TYPE_MONSTER)
		and (ft>0 or c:IsInMainMZone())
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if chk==0 then return Duel.IsExistingMatchingCard(s.spcfilter,tp,LOCATION_DECK,0,1,nil,ft) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,s.spcfilter,tp,LOCATION_DECK,0,1,1,nil,ft)
...
function s.filter(c)
	return c:IsSetCard(0x291) and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local ct=Duel.GetMatchingGroupCount(Card.IsSetCard,tp,LOCATION_MZONE,0,nil,0x291)+1
