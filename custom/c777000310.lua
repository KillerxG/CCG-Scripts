...
function s.initial_effect(c)
	...
	e5:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x297))
	e5:SetValue(s.atkval)
	c:RegisterEffect(e5)
end
--(2)Normal Summon
function s.nscon(e,tp,eg,ep,ev,re,r,rp)
	local ct=Duel.GetMatchingGroupCount(aux.FilterFaceupFunction(Card.IsSetCard,0x297),tp,LOCATION_MZONE,0,nil)
...
function s.nsfilter(c)
	return c:IsSetCard(0x297) and c:IsSummonable(true,nil)
end
...
function s.pctfilter(c,tp)
  return c:IsFaceup() and c:IsSetCard(0x297) and c:GetSummonPlayer()==tp
end
