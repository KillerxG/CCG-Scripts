...
function s.spfilter(c,tp)
	return c:IsSetCard(0x307) and c:IsControler(tp) and not c:IsCode(id)
end
...
function s.cfilter(c,e,tp,sc)
	return c:IsSetCard(0x307) and c:IsType(TYPE_LINK)
end
function s.spcon2(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE,0,1,nil)
end
function s.tdfilter(c)
	return c:IsSetCard(0x307) and c:IsAbleToDeck() and c:IsType(TYPE_MONSTER) and not c:IsCode(id)
end
