...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsSetCard,0x308),tp,LOCATION_MZONE,0,1,nil)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,1,tp,tp,false,false,POS_FACEUP)>0
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,777001885,0,TYPES_TOKEN,1000,1000,4,RACE_BEASTWARRIOR,ATTRIBUTE_DARK)
		and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		Duel.BreakEffect()
		local token=Duel.CreateToken(tp,777001885,0,TYPES_TOKEN,1000,1000,4,RACE_BEASTWARRIOR,ATTRIBUTE_DARK)
