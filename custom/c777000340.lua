...
function s.initial_effect(c)
	--Add "Asuna" Arch
	c:AddSetcodesRule(0x297b)
	--Synchro Material
	Synchro.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x297),1,1,Synchro.NonTuner(nil),1,99,s.matfilter)
	c:EnableReviveLimit()
...
function s.matfilter(c,scard,sumtype,tp)
	return c:IsType(TYPE_MONSTER,scard,sumtype,tp) and c:IsSetCard(0x297b,scard,sumtype,tp)
end
--(1)Activate "SAO" Field Spell
...
function s.filter(c,tp)
	return c:IsSetCard(0x297) and c:IsType(TYPE_FIELD) and c:GetActivateEffect():IsActivatable(tp,true,true)
end
...
function s.tunfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x297) and not c:IsType(TYPE_TUNER)
end
